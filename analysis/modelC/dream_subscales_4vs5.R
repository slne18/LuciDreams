...
script_dir <- local({
  ...
  script_path <- if (length(file_arg) > 0) sub("^--file=", "", file_arg[1]) else "dream_subscales_4vs5.R"
  normalizePath(dirname(script_path), winslash = "/", mustWork = FALSE)
})
source(file.path(script_dir, "..", "glmm_common.R"))

ctx <- init_glmm_script("dream_subscales_4vs5.R")
...
raw_cols <- c("condition", "pid", "lucid_state", dream_subscale_outcomes)
model_df <- df[stats::complete.cases(df[, raw_cols]), , drop = FALSE]
model_df <- model_df[as.integer(model_df$lucid_state) == 0, , drop = FALSE]
model_df <- model_df[as.integer(model_df$condition) %in% 4:5, , drop = FALSE]

model_df$condition <- factor(
  as.integer(model_df$condition),
  levels = 4:5,
  labels = unname(condition_labels)
)
model_df$condition <- stats::relevel(model_df$condition, ref = "condition_4")

log_cat("Rows before filtering:", nrow(df), "\n")
log_cat("Rows used in model C (non-lucid nights, conditions 4 vs 5):", nrow(model_df), "\n")
...
for (outcome_col in dream_subscale_outcomes) {
  ...
  linear_result <- run_glmm_model(
    ...
    model_label = paste0("dream_subscales_4vs5_", slug, "_linear"),
    results_dir = ctx$results_dir,
    timestamp = ctx$timestamp,
    summary_title = paste0(
      "Model C (linear) — ", outcome_col,
      " ~ condition + (1 | pid); non-lucid nights; reference = condition_4"
    )
  )
  ...
  ordinal_result <- run_ordinal_clmm(
    ...
    data = ordinal_df,
    model_label = paste0("dream_subscales_4vs5_", slug, "_ordinal"),
    results_dir = ctx$results_dir,
    timestamp = ctx$timestamp,
    summary_title = paste0(
...
if (length(all_coefs) > 0) {
  ...
  combined_path <- file.path(
    ctx$results_dir,
    paste0("dream_subscales_4vs5_all_coefficients_", ctx$timestamp, ".csv")
  )
