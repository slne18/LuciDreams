...
public class VolumeButtonsPlugin extends Plugin {
    ...
    @PluginMethod(returnType = PluginMethod.RETURN_PROMISE)
    public void getCurrentVolume(final PluginCall call) {
        try {
            ...
            double normalized = (max > 0) ? ((double) current / (double) max) : 0.0d;
            JSObject ret = new JSObject();
            ret.put("value", normalized);
            ret.put("current", current);
            ret.put("max", max);
            call.resolve(ret);
        } catch (Exception ex) {
            call.reject("Failed to get current volume", ex);
        }
    }
